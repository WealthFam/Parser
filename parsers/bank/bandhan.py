...
class BandhanSmsParser(BaseSmsParser):
    """
    Parser for Bandhan Bank SMS Alerts.
    """
    name = "Bandhan"
    ...
    def get_patterns(self) -> List[TransactionPattern]:
        return [
            # UPI
            TransactionPattern(
                regex=re.compile(r"(?i)UPI\s*transaction\s*of\s*(?:INR|Rs\.?)\s*([\d,]+\.?\d*)\s*(?:to|from)\s*(.*?)\s*(?:from|to)\s*A/c\s*([\d*xX]*\d+).*?successful\.\s*(?:Ref\s*No[:\.\s]*(\d+))?.*?(?:Avail\.\s*Bal[:\.\s]*(?:INR|Rs\.?)\s*([\d,]+\.?\d*))?", re.IGNORECASE),
                confidence=1.0,
                txn_type="DEBIT",
                field_map={"amount": 1, "recipient": 2, "mask": 3, "ref_id": 4, "balance": 5}
            ),
            # Generic
            TransactionPattern(
                regex=re.compile(r"(?i)(?:INR|Rs\.?)\s*([\d,]+\.?\d*)\s*has\s*been\s*(debited|credited).*?A/c\s*No\.?\s*([\d*xX]*\d+)\s*on\s*([\d-]+).*?(?:for|by)\s*(.*?)\.\s*(?:Avail\.\s*Bal[:\.\s]*(?:INR|Rs\.?)\s*([\d,]+\.?\d*))?", re.IGNORECASE),
                confidence=0.9,
                txn_type="DEBIT",
    ...
    def parse_with_confidence(self, content: str, date_hint: Optional[datetime] = None) -> List[ParsedTransaction]:
        ...
        for tx in results:
            if "credited" in content.lower():
                tx.type = "CREDIT"
            elif "debited" in content.lower():
                tx.type = "DEBIT"
        return results

    def can_handle(self, sender: str, message: str) -> bool:
        return "bandhan" in sender.lower() or "bandhan" in message.lower()
